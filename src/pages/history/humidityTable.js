...
import { getHumiditySensorData, getPhSensorData, getTempSensorData } from "../../services/sensores";
...
export const HumidityTable = () => {
    ...
    const {
        ...
        prev,
        jump,
    } = usePagination(humidityData, 5);

    useEffect(() => {
        const obtenerDatos = async () => {
            try {
                const response = await getHumiditySensorData();
                if (response.success) {
                    setHumidityData(response.data.humiditySensor);
                } else {
                    console.log("Error")
                }
            } catch (error) {
    ...
    const handleExport = () => {
        ...
        exportDataToExcel(humidityData, options);
      };
        ...
        <>
            ...
            <div className="table-container mt-1">
                <table className="content-table">
                    ...
                    <tbody>
                        {paginatedUsers.map((humidityData, index) => (
                            <tr key={humidityData._id}>
                                <td>{(currentPage - 1) * 5 + index + 1}</td>
                                <td>{humidityData.id_sensor}</td>
                                <td>{humidityData.values}</td>
                                <td>{humidityData.timestamp}</td>
                                <td>{humidityData.location.coordinates}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
